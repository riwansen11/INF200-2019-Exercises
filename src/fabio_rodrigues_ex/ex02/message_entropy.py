...
def letter_freq(txt):
    """ Returns a dictionary = {(key : value)}, from the input string,
    with key = letters and value = the number of occurrences of the
...
def entropy(message):
    ...
    n_i: number of occurrences of letter i (i is the UTF-8 code for
    the letter)
    ...
    return -1 * sum([p_i[i] * log2(p_i[i]) for i in range(len(p_i))])
